...
public class HelloTriangle extends Demo {
     ...
     * Swapchain state
     */
    private static class SwapchainDimensions {
    ...
    private static boolean validate_extensions(PointerBuffer required, VkExtensionProperties.Buffer available) {
        for (int i = 0; i < required.capacity(); ++i) {
            ...
            for (VkExtensionProperties available_extension : available) {
                if (available_extension.extensionNameString().equals(memASCII(required.get(i)))) {
                    found = true;
                    break;
                }
            }
     ...
     */
    void init_instance(PointerBuffer requiredExtensions) {
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            VkApplicationInfo app = VkApplicationInfo.calloc(stack)
                ...
                .pEngineName(stack.ASCII("Vulkan Samples"))
    ...
    void init_device() {
        try (MemoryStack stack = stackPush()) {
            ...
            IntBuffer gpu_count = stack.mallocInt(1);
            VK_CHECK(vkEnumeratePhysicalDevices(context.instance, gpu_count, null));
            ...
            PointerBuffer gpus = stack.mallocPointer(gpu_count.get(0));
            ...
            VK_CHECK(vkEnumerateDeviceExtensionProperties(context.gpu, (ByteBuffer)null, device_extension_count, null));

            PointerBuffer required_device_extensions;
            try (VkExtensionProperties.Buffer device_extensions = VkExtensionProperties.calloc(device_extension_count.get(0))) {
                VK_CHECK(vkEnumerateDeviceExtensionProperties(context.gpu, (ByteBuffer)null, device_extension_count, device_extensions));
    ...
    void init_vertex_buffer() {
        ...
        int buffer_size = vertices.length * Float.BYTES;
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            if (buffer_alloc_info.pMappedData() != VK_NULL_HANDLE) {
                ...
                memByteBuffer(buffer_alloc_info.pMappedData(), buffer_size)
                    .asFloatBuffer()
                    .put(vertices);
            } else {
    ...
    void init_per_frame(PerFrame per_frame) {
        try (MemoryStack stack = stackPush()) {
            ...
            PointerBuffer pp = stack.mallocPointer(commandBufferCount);
    ...
    void init_swapchain() {
        try (MemoryStack stack = stackPush()) {
            ...
            VkSurfaceFormatKHR format = select_surface_format(context.gpu, context.surface, Collections.emptyList());
    ...
    private static long compile_shader(String fileName, int shaderType, long compiler, long options) {
     ...
     */
    long load_shader_module(long spv) {
        try (MemoryStack stack = stackPush()) {
    ...
    int acquire_next_image(IntBuffer image) {
        try (MemoryStack stack = stackPush()) {
            ...
            if (context.per_frame[image.get(0)].queue_submit_fence != VK_NULL_HANDLE) {
                vkWaitForFences(context.device, context.per_frame[image.get(0)].queue_submit_fence, true, UINT64_MAX);
                vkResetFences(context.device, context.per_frame[image.get(0)].queue_submit_fence);
            }

            if (context.per_frame[image.get(0)].primary_command_pool != VK_NULL_HANDLE) {
                vkResetCommandPool(context.device, context.per_frame[image.get(0)].primary_command_pool, 0);
            }
     ...
     */
    void render_triangle(int swapchain_index) {
    ...
    public static void main(String[] args) throws IOException {
        try (
            Window window = new GLFWWindow(1280, 720, "Vulkan Samples: Hello Triangle");
